...
nohup python3 models/entlm_crf_rfc.py                        			\
			...
			--patience 10         \
			--cuda_device ${ID}       \
			--handmaded_label_map_path ${MAP_PATH} \
			--redundancy \
			>log_entlm_crf_ds+hm_re_lr1e-5_${PROTOCOL}.txt 2>&1
done
python3 scripts/read_results.py --logs_name log_entlm_crf_ds+hm_re_lr1e-5