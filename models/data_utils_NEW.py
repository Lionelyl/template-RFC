...
def translate_entlm(y, tag2index):
    index2tag = {v:k for k,v in tag2index.items()}
    y_new = []
    for seq in y:
        y_new.append([index2tag[i] for i in seq])
        #print([id2tag[i] for i in seq])
    return y_new
...
def add_label_token_bert(model, tokenizer, procotol, label_map_path, handmade_lmp, tag2index, redundancy=False):

    # label_map = json.load(open(label_map_path, 'r'))
    if label_map_path:
        label_map = json.load(open(label_map_path, 'r'))
    else:
        label_map = label_map_utils.get_label_map(procotol, k=6, filter_ratio=0.6)

    if handmade_lmp:
        h_label_map = json.load(open(handmade_lmp, 'r'))
        for k, v in h_label_map.items():
            label_map[k] += v
            if not redundancy:
                label_map[k] = list(set(label_map[k]))

    # print(label_map)

    sorted_add_tokens = sorted(list(label_map.keys()), key=lambda x: len(x), reverse=True)
    tokenizer.add_tokens(sorted_add_tokens)
    num_tokens, _ = model.chunk_bert.bert.embeddings.word_embeddings.weight.shape
    model.chunk_bert.resize_token_embeddings(len(sorted_add_tokens)+num_tokens)

    for token in sorted_add_tokens:
        if token.startswith('B-') or token.startswith('I-'):  # 特殊字符
            # print(f'{token} : {label_map[token]}')
            index = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(token))
            tag2index[token if token[2:]!='O' else 'O'] = index[0]
            if len(index)>1:
            ...
            if isinstance(label_map[token], list):
                ...
                indexes = tokenizer.convert_tokens_to_ids([label_map[token]])
            indexes = [id for id in indexes if id != 100]

            print(f'{token} : {tokenizer.convert_ids_to_tokens(indexes)}')

            embed = model.chunk_bert.bert.embeddings.word_embeddings.weight.data[indexes[0]]

            # Calculate mean vector if there are multiple label words.
            for i in indexes[1:]:
                embed += model.chunk_bert.bert.embeddings.word_embeddings.weight.data[i]
            embed /= len(indexes)
            model.chunk_bert.bert.embeddings.word_embeddings.weight.data[index] = embed

    return tokenizer


def bert_sequences(X, y, max_chunks, max_tokens, id2word, tokenizer, tag2index, id2tag, is_y_with_tag_index=True):
    ...
    for x, y_ in zip(X, y):
        ...
        # y_word = [labelwords[i] for i in y_ ]
        # y_out = tokenizer(y_word)
        # y_ = [x[1] for x in y_out['input_ids']]
        if is_y_with_tag_index:
            y_ = [tag2index[id2tag[i]] for i in y_]
...
def bert_templates(templates, bert_model):
    tokenizer = AutoTokenizer.from_pretrained(bert_model)
    # max_tokens = 0
    # for t in templates:
    #     max_tokens = max(max_tokens, len(t))
    encode_templates = tokenizer.batch_encode_plus(templates)
    return encode_templates
